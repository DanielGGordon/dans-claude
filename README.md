...
│       └── orchestration-smoke-model-runner.js  # 9 parallel model-runner agents: preferred non-Claude ids + every --task-type; asserts nonce AND resolved `MODEL:` id
...
How Claude Code sessions on this machine reach non-Anthropic models (gpt-5.5/5.6/6-astra via the Codex CLI, composer-2.5 / grok-4.6 (default grok; 4.5 legacy) / glm-5.2 via the Cursor CLI — both on subscription-seat auth, no API keys), and how that stays deterministic.

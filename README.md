...
├── bin/
│   ├── model-run.sh         # THE single entrypoint for non-Claude model calls: canonical flags, timeouts, one auto-retry on transient transport errors, distinct exit codes (64 bad-id / 73 transport-after-retry / 75 auth-quota / 124 timeout); accepts <model-id> or --task-type bulk|cheap|recency|x-recency|second-review|fable-fallback; pins codex reasoning effort from routes.tsv (MODEL_RUN_EFFORT overrides); backend `xai` = the direct xAI Responses API over curl (grok with real X + web search, `store:false`, key from env/~/.profile, measured `xai-tools x_search=<n>` stderr line; `--xai-models` = zero-token catalog read)
...
│   ├── model-scout.sh       # The DAILY unattended routing maintainer (cron, installed by install.sh): in its own worktree, collects zero-token signals (CLI versions, routecheck, catalog-drift + --unrouted), runs headless opus on scout/prompt.md (grok x-recency research with real X search, web-only recency fallback = run DEGRADED → Claude WebSearch → catalogs → edit routes/docs → routecheck → gpt-6-astra review), gates the diff and opens/updates ONE `claude/model-scout-*` PR; always cleans up its test chats; state in ~/.claude/model-scout/
...
│   ├── route-health-banner.sh  # SessionStart: warns (from cached ~/.claude/route-health.txt) when routecheck last failed or is >14d stale — never runs tests itself; warns when claude/codex/cursor-agent changed since the last routecheck (cli-fingerprint.sh vs ~/.claude/route-health-tools.txt); also runs catalog-drift.sh --cached and prints one line when a catalog has a newer grok/composer/glm/gpt than routes.tsv (or a routed id vanished); prints one `[model-scout]` line only when the daily scout failed, was DEGRADED (research had no X search), is >36h stale (or installed >36h ago and never recorded a run), has a PR awaiting review, or left an unpublished `--no-pr` commit on a still-existing local branch (reads ~/.claude/model-scout/last-run.json, no network)
...
codex CLI / cursor-agent    subscription-seat CLIs (never invoked raw)
  / curl → api.x.ai         xai backend: direct xAI Responses API (x-recency)
```
...
- **`bin/model-run.sh`** — the only way models get invoked. Takes `<model-id>` or `--task-type bulk|cheap|recency|x-recency|second-review|fable-fallback` (the table resolves the id — the LLM only picks a class), plus a prompt **file** (never inline) and optional workdir. Codex reasoning effort comes from the table's 4th column (`gpt-6-astra` → `high`, because its catalog default is `low`); `MODEL_RUN_EFFORT` overrides per call. `MODEL_RUN_EPHEMERAL=1` marks a **test** call: codex gets `--ephemeral` (no rollout / thread row, so nothing in `codex resume` or the Codex app); cursor has no equivalent, so test callers pass a throwaway workdir and run `bin/test-chat-cleanup.sh` for it. The `xai` backend (`x-recency` → `grok-4.7-xsearch`) is a `curl` to `api.x.ai/v1/responses` with tools `web_search` + `x_search` and `store: false`; it prints the answer + a `Sources:` URL list, and one stderr line `model-run: xai-tools x_search=<n> web_search=<n> ... cost_usd=<n>` from the response's usage (proof X was searched); `XAI_API_KEY` comes from the env, else from what `~/.profile` exports (as for cron), and is never put in argv. Transient transport errors get one automatic retry with backoff. Distinct exit codes: `64` bad/retired id · `73` transport error persisting after retry · `75` auth/quota (agents must stop and surface, never substitute a model) · `124` timeout.
- **`bin/routes.tsv`** — edit THIS when the model catalog changes; script errors, docs, and tests all derive from it. Then run `routecheck`. Row shape: `model <id> <backend> [<col4>]` (col4: codex → optional reasoning effort; xai → required xAI API model id, e.g. `grok-4.7-xsearch xai grok-4.7`); also `retired <old> <successor>`, `task <type> <id>`, and `ignore <glob> <reason>` (a catalog id seen and deliberately not routed — see "Catalog drift detection").
...
- **Enforcement (hooks)** — `route-guard.sh` (PreToolUse on Bash) denies raw `codex exec` / headless `cursor-agent` calls, raw `curl` to the xAI generation endpoints (`api.x.ai/v1/responses` etc.; `GET /v1/models` is allowed) and retired model ids with a structured reason pointing at the blessed path; command-position matching with quoted-prose exemption, `bash -c` smuggling covered. Applies to subagents too. `route-health-banner.sh` (SessionStart) surfaces cached routecheck failures/staleness at session start without running tests, warns when claude/codex/cursor-agent changed since the last routecheck (`bin/cli-fingerprint.sh`, zero cost), and runs the catalog-drift check (below) from a 24h cache.
...
`tests/routecheck.sh` (alias `routecheck`) verifies the whole layer: Tier 0 hook unit tests (deny/allow cases incl. bypass regressions), a mock-backend tier (PATH-shimmed fake codex/cursor-agent/curl proving the error taxonomy deterministically — auth→75, transport→73 after retry, transient→recovers, quoted-prose→no false positive; for xai also quota 429→75, timeout→124, a connect that never completes→73 after retry, no key→75, the `--xai-models` catalog read (401 / credits-429→75, plain rate-limit 429→73), and the request shape: `store:false`, `web_search`+`x_search`, `from_date`, key sent as a header but never in argv — plus the catalog-drift detector against a fake future catalog), zero-token table/auth/arg-parsing checks, the live catalog-drift check (vanished id FAIL / newer version WARN), then live smokes: a nonce echo for **every** routes.tsv row through model-run.sh (tested path = used path, ~100 tokens/route; the xai route's smoke asks for the sum of two per-run random numbers instead of a nonce echo — grok-4.7 on the xAI API refuses "output exactly this token" prompts — tells grok not to search, and is SKIPPED with a WARN — never a FAIL — when no `XAI_API_KEY` is in the env or `~/.profile`, the same lookup cron gets; a key that is present but rejected FAILs `auth:xai`, a zero-token `GET /v1/models` check — `ROUTECHECK_XAI_SOFT=1`, set only by the model scout's publish gate, downgrades those xai failures to WARNs; the smoke accepts the sum with digit-group separators, e.g. `231,456`) plus one **artifact smoke per backend** — the model must actually write a file, catching tool-execution/sandbox breakage that text round-trips can't see. `--no-live` runs just the free tiers. Full runs write `~/.claude/route-health.txt` for the SessionStart banner, plus `~/.claude/route-health-tools.txt` — the claude / codex / cursor-agent builds the run verified against (`bin/cli-fingerprint.sh --versions`). Rule: a FAILing route means the policy files are wrong — fix the id/syntax or remove the model; never leave a documented route broken.
...
- **xAI (backend `xai`)** — read via `model-run.sh --xai-models` (`GET /v1/models`, zero tokens) and checked for **vanished only**, against each xai row's API model id (column 4): xAI's raw API ids defeat version-max (`grok-4.20` predates `grok-4.7`), and new groks already surface through the Cursor catalog. No `XAI_API_KEY` (env or `~/.profile`) = skipped silently.
...
One run:
1. **flock** single instance; log `~/.claude/model-scout/logs/<date>.log` (+ `.report.md` / `.grok.md` / `.grok-fallback.md` / `.review.md` / `.patch`, 30 days kept); cron.log gets one start and one end line.
...
4. **Agent**: `claude -p --no-session-persistence --model opus --effort high --permission-mode bypassPermissions` plus `--disallowedTools` deny rules (git commit/push, `gh pr` writes, crontab, install.sh) and refusing git commit/push hooks in its env, `BASH_MAX_TIMEOUT_MS` raised to 30 min for the ~10-min grok/astra calls, cwd = the worktree, prompt = `scout/prompt.md` + the signals, hard timeout `MODEL_SCOUT_TIMEOUT` (5400s). It must research with grok **first** — `model-run.sh --task-type x-recency` (the direct xAI API with real X + web search, `x_search` from the research window's start; X hot takes from practitioners and lab staff with post URLs, plus web) — then confirm every claim via Claude WebSearch/WebFetch of primary sources, reconcile with the live catalogs, then update routes.tsv / docs / `route-guard.sh` RETIRED / the routecheck mock fixture / `scout/evaluated.tsv`, re-run routecheck (repairing invocations a CLI update broke, or removing the route), get a `--task-type second-review` (gpt-6-astra) review, and write `scout/last-report.md`. It writes a status line and the grok output to files the wrapper checks. The gate is **measured**, not self-reported: model-run's `model-run: xai-tools x_search=<n>` line (from the xAI response's usage). x-recency with ≥1 x_search call = full research. If x-recency fails (exit 75/73/124, anything else, or 0 x_search calls after one retry) the agent falls back to `--task-type recency` (Cursor grok, web only), announces it in the report, and the run is recorded **`degraded`**, not failed — with the reason named (`XAI_API_KEY rejected` / `not set` / credits / transport / timeout / 0 X searches). No grok output, output that isn't model-run's, a missing fallback, or both passes failing = BLOCKED (failed). A rejected / out-of-credit / rate-limited key also FAILs the live routecheck's `auth:xai` (and the xai smoke); when those are the ONLY failures the run is still `degraded`, not failed, and the PR is still published — the publish gate runs `routecheck --no-live` with `ROUTECHECK_XAI_SOFT=1`, which turns just those xai failures into WARNs (the live banner's own routecheck stays strict). X gets its own window: `last_x_success` in `last-run.json` advances only on a measured X search, and the next x-recency's `from_date` (`$MODEL_SCOUT_X_SINCE`) starts there — capped at 30 days — so a degraded run's gap is searched later rather than skipped.
5. **Gate + publish**: only routing-layer files may change (routes.tsv, model-run.sh, catalog-drift.sh, cli-fingerprint.sh, routecheck, the workflow smokes, route-guard, the banner, model-runner.md, model-selection.md, model-usage.md, README.md, system-map.md, `scout/evaluated.tsv` / `last-report.md`); anything else — install.sh, the cleanup scripts, the prompt, this script — is reverted and logged. Every changed `.sh` must pass `bash -n` and `routecheck --no-live` must pass (with `ROUTECHECK_XAI_SOFT=1`: an xai key/credit problem is the account, not the diff, so it WARNs there). Then commit (`Co-Authored-By` the model that actually ran), push (SSH, falling back to HTTPS via `gh auth git-credential`), and `gh pr create --base master` with the report as the body — or push to / `gh pr edit` the open scout PR after re-checking it is still OPEN (MERGED mid-run: the commit is replayed onto `origin/master` for a fresh PR; CLOSED or unknown: kept on a local branch, run failed). Never pushes to master.
6. **Always (EXIT trap)**: first retry every sweep queued in `~/.claude/model-scout/pending-cleanup.tsv` by an earlier failed run, then `test-chat-cleanup.sh --since <run start> --marker <run marker>` for the worktree + every throwaway workdir, then `t3-purge-test-threads.sh --apply` (if either fails — e.g. t3code down — the sweep is queued and the run recorded failed) (both run from the script's own `bin/`, never from the worktree the agent could edit), remove the worktree, the headless session's transcript-less `~/.claude/projects/<enc(worktree)>/` and temp dirs, then write `~/.claude/model-scout/last-run.json` (`{date, status: no-change|pr|local-commit|degraded|failed, pr_url, open_pr, summary, log, cleanup, last_success, last_x_success, …}`; `open_pr` keeps an open scout PR visible across later no-change runs; `local-commit` = a `--no-pr` run (explicit, or forced by `--base`) that committed to a local `branch`, unpublished (the banner prints an info line while that branch exists); `degraded` = worked and published, but research had no X search or the live routecheck failed only on the xai key (the banner prints a `WARNING last run DEGRADED` line); `failed` also covers runs that worked but left something actionable — incomplete cleanup, a live routecheck still failing — and wins over `degraded`).

No test chat may survive: every model call runs with `MODEL_RUN_EPHEMERAL=1` in a `mktemp` workdir under the run's scratch dir, every prompt carries the run marker `MODEL-SCOUT-<date>-<hex>` (shared with routecheck via `ROUTECHECK_MARKER`), and the cleanup scripts are the backstop. Auth/quota errors (model-run exit 75, claude login/usage limits) fail the run loudly into `last-run.json` — the scout never substitutes a model. The one sanctioned, announced fallback is x-recency → recency above, and it is recorded `degraded`. The SessionStart banner surfaces the result as one `[model-scout]` line, only when there's something to do.
